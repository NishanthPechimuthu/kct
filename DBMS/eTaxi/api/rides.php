...
require_once '../config/config.php'; // Use centralized config
...
try {
    if ($action === 'estimate') {
        if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'passenger') {
            ...
            echo json_encode(['success' => false, 'message' => 'Unauthorized for this action.']);
            exit;
        }
        ...
        $stmt = $pdo->prepare("SELECT s.*, sp.* FROM Subscriptions s JOIN Subscription_Plans sp ON s.plan_id = sp.plan_id WHERE s.passenger_id = ? AND s.is_active = 1 AND CURDATE() BETWEEN s.start_date AND s.end_date");
        ...
        if ($sub) {
            ...
            if ($sub['rides_taken'] < $sub['max_rides']) {
                if ($distance <= $sub['max_km_per_ride']) {
                    $fare = 0; // Ride is fully covered by subscription
                } else {
                    // Ride is longer than plan allows, charge for extra KMs
                    $extra_km = $distance - $sub['max_km_per_ride'];
                    $fare = ($extra_km * PER_KM_CHARGE) * ($sub['extra_km_charge_percent'] / 100);
                }
            } else {
                // User is out of rides, charge a percentage of the normal fare
                $normal_fare = BASE_FARE + ($distance * PER_KM_CHARGE);
                $fare = $normal_fare * ($sub['extra_ride_charge_percent'] / 100);
            }
        } else {
            // No active subscription, calculate normal fare
